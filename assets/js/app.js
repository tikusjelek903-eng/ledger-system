...
function renameLedgerToHistoryTransaction() {
  // Ubah judul dan subtitle halaman
  if (typeof meta !== "undefined" && meta.ledger) {
    meta.ledger = [
      "History Transaction",
  ...
  // Ubah nama menu sidebar tanpa menghilangkan ikon
  const ledgerMenu = document.querySelector(
    '[data-page="ledger"]'
  );
  ...
  // Ubah judul panel di halaman Ledger
  const ledgerPage = document.getElementById("ledgerPage");

  if (ledgerPage) {
    const panelTitle =
      ledgerPage.querySelector(".panel-head h3");

    const panelSubtitle =
      ledgerPage.querySelector(".panel-head p");
  ...
  // Kalau halaman Ledger sedang terbuka, langsung ganti judul atas
  if (ledgerPage?.classList.contains("active")) {
    const pageTitle = document.getElementById("pageTitle");
    const pageSubtitle =
      document.getElementById("pageSubtitle");

    if (pageTitle) {
      pageTitle.textContent = "History Transaction";
    }
...
if (document.readyState === "loading") {
  document.addEventListener(
    "DOMContentLoaded",
    renameLedgerToHistoryTransaction
  );
} else {
  renameLedgerToHistoryTransaction();
}
